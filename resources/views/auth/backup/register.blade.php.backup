    ...
    <div
        ...
        <div class="max-w-md bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6">
            ...
            <div class="text-center mb-6">
                ...
                <p class="mt-1 text-sm text-gray-600">Bergabunglah dengan Makna Academy</p>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const disposableDomains = [
                ...
                'throwaway.email', 'mohmal.com', 'mailtemp.info'
            ];
            ...
            function validateEmail(email) {
                ...
                if (disposableDomains.includes(domain.toLowerCase())) {
                    showMessage('Email sementara/disposable tidak diperbolehkan', 'error');
                    return;
                }

                // Check gmail with numbers only
                if (domain.toLowerCase() === 'gmail.com') {
                    const localPart = email.split('@')[0];
                    if (/^\d+$/.test(localPart)) {
                        showMessage('Email dengan format angka saja tidak diperbolehkan', 'error');
                        return;
                    }
                }

                // Show success message for valid format
                showMessage('Format email valid', 'success');
            }

            function showMessage(message, type) {
                emailMessage.textContent = message;
                emailMessage.classList.remove('hidden', 'text-red-600', 'text-green-600');
                emailMessage.classList.add(type === 'error' ? 'text-red-600' : 'text-green-600');
            }
        });
    </script>
</body>
